...
public class LightningBoltScript : Bullet
{
...
public override void Init(int damage, GameObject Position, GameObject Target)
{
	this.damage = damage;
...
{
	if(EndObject!=null) EndObject.GetComponent<Enemy>().TakeDamage(damage);	
}
	...
	if (countDown >= lifeTime) Destroy(gameObject);
	//else if(countDown <= lifeTime && EndObject == null) Destroy(gameObject);
